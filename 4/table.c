...
void char_row_print(char_row *r, int max_len) {
  for (int i = 0; i < r->len; ++i)
    printf("| %-*s ", max_len, r->data[i] ? r->data[i] : "");
  printf("|\n");
}
...
void print_row_separator(int side, int n, int size) {
  for (int i = 0; i < n; ++i) {
    printf("%s", i == 0 ? (side ? "." : "|") : "+");
...
void column_print(column *head) {
  int all_max_len = 0, i_max = 0;
  column *ptr = head;
  while (ptr != NULL) {
    all_max_len = max(all_max_len, char_row_max_len(ptr->data));
    i_max = max(i_max, ptr->data->i);
    ptr = ptr->next;
  }
  ptr = head;
  while (ptr != NULL) {
    print_row_separator(ptr == head, i_max, all_max_len);
    char_row_print(ptr->data, all_max_len);
    ptr = ptr->next;
  }
  print_row_separator(1, i_max, all_max_len);
}
